...
The next step is to map the reads back to the transcriptome to establish expression values. We'll do this with RSEM -- and afterwards, these expression levels will be used to help clean and filter the transcriptome. I'm going to try running this using Trinity's built-in perl scripts, as described [on their Github page](https://github.com/trinityrnaseq/trinityrnaseq/wiki/Trinity-in-Docker). Because these need sudo permissions, full paths, and other tricky things, I'm not going to run them in a script but rather run them using `screen` on the RCC.

The first step is to prepare the reference for alignment and abundance estimation. 

```{bash, eval=FALSE}
sudo docker run -v/home/rccuser/:/home/rccuser trinityrnaseq/trinityrnaseq /usr/local/bin/util/align_and_estimate_abundance.pl --transcripts /home/rccuser/pipefish_sbge/stigmatopora/results/trinity_nigra_gonads.Trinity.fasta --est_method RSEM --aln_method bowtie2 --trinity_mode --prep_reference
```


This results in the Trinity RSEM files.

Next up, run the alignment and abundance estimation. Running this in `screen -S rsem`. Without the command-line wrapper in it (the use of `/bin/sh -c` followed by the actual command), none of the outputs get saved. 
...
sudo docker run -v/home/rccuser/:/home/rccuser trinityrnaseq/trinityrnaseq /bin/sh -c "cd /home/rccuser/pipefish_sbge/stigmatopora/results && /usr/local/bin/util/align_and_estimate_abundance.pl --transcripts /home/rccuser/pipefish_sbge/stigmatopora/results/trinity_nigra_gonads.Trinity.fasta --est_method RSEM --aln_method bowtie2 --trinity_mode --seqType fq --samples_file /home/rccuser/pipefish_sbge/stigmatopora/nigra_gonads_trinity_rcc.txt --output_dir /home/rccuser/pipefish_sbge/stigmatopora/results/rsem_gonads --thread_count 12"

```

This took several days to run and saved each sample's data to its own directory.

Finally, I need to run the following code to generate the summary matrix. Be sure to create the list of isoforms.results files first! (also, this needs to be the isoforms files, not the genes files).

```{bash, eval=FALSE}
ls */*isoforms.results > isoforms.results_list.txt

sudo docker run -v/home/rccuser/:/home/rccuser trinityrnaseq/trinityrnaseq /bin/sh -c "cd /home/rccuser/pipefish_sbge/stigmatopora/results && /usr/local/bin/util/abundance_estimates_to_matrix.pl --est_method RSEM \
      --gene_trans_map /home/rccuser/pipefish_sbge/stigmatopora/results/trinity_nigra_gonads.Trinity.fasta.gene_trans_map \
      --quant_files genes.results_list.txt \
      --name_sample_by_basedir"
```


#### Transcriptome statistics based on expression categories

Trinity also provides scripts to calculate ExN50 statistics, which calculates N50s for genes in different categories of top percentages of normalized expression. 

```{bash, eval=FALSE}
sudo docker run -v/home/rccuser/:/home/rccuser trinityrnaseq/trinityrnaseq /bin/sh -c "cd /home/rccuser/pipefish_sbge/stigmatopora/results && /usr/local/bin/util/misc/contig_ExN50_statistic.pl \
        /home/rccuser/pipefish_sbge/stigmatopora/results/RSEM.isoform.TMM.EXPR.matrix  \
        /home/rccuser/pipefish_sbge/stigmatopora/results/trinity_nigra_gonads.Trinity.fasta | tee /home/rccuser/pipefish_sbge/stigmatopora/results/ExN50_output"
```


## Differential expression analysis

First we need to read in the data. I will use tximport to enable seamless interactions between RSEM and DESeq2 -- but this has to be run on the RCC because that's where the files are (currently). So I'm saving the `txi.rsem` object as an RDS to move over to my local machine to then be able to be able to run things locally.


```{r tximport, eval=FALSE}
library(tximport)
dirs<-list.dirs()
dirs<-dirs[grep("SN",dirs)]
files<-unlist(lapply(dirs,list.files, 
                     pattern=".genes.results",
                     full.names=TRUE))

txi.rsem <- tximport(files, type = "rsem", txIn = FALSE, txOut = FALSE)
saveRDS(txi.rsem,"txi.rsem_gonads.RDS")
```
...
# get expression data
txi.rsem<-readRDS("txi.rsem_gonads.RDS")
# get the first few rows just to match up names
cts<-read.table("RSEM.gene.counts.matrix",nrows = 2) 

# make sure sample info is in correct order
samples <- read.table("../nigra_gonads_trinity.txt",
                      header=FALSE)
rownames(samples)<-samples$V2
samples<-samples[order(samples$V2, colnames(cts)),1:2]
samples$sex<-gsub("(\\w+)_(\\w+)_(\\w+)","\\3",samples$V1)
samples$spp<-gsub("(\\w+)_(\\w+)_(\\w+)","\\1",samples$V1)
samples$tis<-gsub("(\\w+)_(\\w+)_(\\w+)","\\2",samples$V1)

# start DESeq analysis
...
```{r}
# make the plot
vpca<-plotPCA(vsd, intgroup=c("sex"))

plot(vpca$data$PC1, 
     vpca$data$PC2,
     col=sex_cols[vpca$data$sex],
     xlab=vpca$labels$x,
     ylab=vpca$labels$y,
     cex=3,
     pch=c(1,6)[vpca$data$sex],
     lwd=3,
     ylim=c(-30,30))
legend("topleft",
       c("Female","Male"),

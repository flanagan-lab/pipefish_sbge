...
From the heatmap we can see that for all of these top 20 expressed genes the highest expression in found in the gonads, which is different from the other two _Stigmatopora_ spp. and all of the _Syngnathus_ species that had the highest expression in the liver. There are also a few genes that appear to be highly expressed across all of the tissue types. 
...
S39 still appears to be an outlier, and the S32 is clustering outside of any of the organ groups. From the pairs plot we can see that PC1 appears to be explaining differences between the gonad and the other tissues, PC2 appears to be explaining differences between gonadal and somatic tissue.
...
We can then generate a sample-to-sample distances heatmap that will give an overview of the similarities and differences between our samples. The darker the colour, the more similar the samples are. The diagonal line of very dark blue represents the comparison between the same samples.

```{r rm-samples-sample-dist, echo=FALSE, fig.cap="Sample-to-sample distances heatmap showing the similarities and differences between all of the samples. The darker the colour, the more similar the samples are. The diagonal line of very dark blue represents the comparisons between the same samples.", warning=FALSE}
#Calculate the sample-to-sample distances from the transformed count matrix
sampleDists <- dist(t(assay(vsd)))
...
Now we can see that the highest similarities are among samples from the same organ and all organs are clustering within their organ group. 

### PCA plots

```{r rm-samples-pca-pairs-plot, echo=FALSE, fig.cap="Principal components analysis pairsplot showing PCA axis 1 - 4."}
#Generate the PCA data to create the pairs plot, using the PCAtools package
p <- pca(assay(vsd), 
         metadata = colData(dds_SA))
...
From the pairs plot we can see that PC1 appears to be explaining differences between the gonads and the other tissues, PC2 appears to be explaining differences between gonadal and somatic tissues, PC3 seems to account for differences between gonad samples, and in PC4 we can see three gill samples sticking out from all the rest. I am going to plot all of PC2 - 4 against PC1 below for a clearer picture.

Next we can look at PCA plots for our samples to see how our different samples are grouping.
```{r rm-samples-pca-plot, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r pca1v3, echo=FALSE, fig.cap="The same Principal Components Analysis as above but with PC3. This third axis explains 8.63% of the variation, largely attributed to differences between the gonads samples.", warning=FALSE}
...
It looks like maybe some of the gonad samples were mislabelled not just for organ, but also for sex. We see S40 and S23 that are labeled as female samples clustering with other male samples. We also see S26 and S38 that are labelled as male samples clustering with female samples. In the liver samples, we see S37 and S34 which are labelled as male samples clustering with female samples.
...
If we investigate the column of our DESeq dataset, we can see that each sample has both a sex and an organ type attached to it - we will be using these two factors in our multifactor analysis. Prior to running the multifactor analysis, I'm going to remove S39 because of its low mapping rate and clustering away from all other samples. 

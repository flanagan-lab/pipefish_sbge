...
blast_output_merged <- lapply(blast_output_filtered, function(dataframe){
  ...
  merged_info<-data.frame(matrix(ncol=nrow(dataframe)+5))
  for(x in 1:nrow(dataframe)){
    ...
    merged_gene <- cbind(dataframe[x,], gene_id[,c(1,4,5,8,9)])
   #browser()
     merged_info<-rbind(merged_info, gene_id)
  }
  
 output <- merge(dataframe, unique(genes[,c(1,7,9)]), by.x = "prot_id",by.y="seqname", all.x=TRUE)
...
```{r pull-out-gene-products}
# gene products in blast outputs
#sexbiased_products <- lapply(blast_output_merged, function(x){ unique(x$product) })



# all gene products in the genome
#gene_products <- unique(genes$product)

#write.csv(gene_products, 'data/gene_products.csv')
```

After pulling out the unique product information, there are a few candidates for indicators:
* oocyte zinc finger protein * <- there are 24 of these each with different suffixes (fem gon)
* hepatocyte nuclear factor * <- there are 25 of these each with different suffixes (potentially liver, but Huang & Weng 2010 found some HNF families detected in teleost gonads)
* HNF-6 <- contributes to the control of the expression of liver-enriched transcription factors, and there are 2 of these (liver, either sex)
* Vitellogenin * <- there are 3 of these with different suffixes, but vtg1, vtg4, and vtg5 overlap in males and females (Zheng et al., 2013)
* estrogen-related receptor * <- there are 29 of these each with different suffixes (fem gon)
* progestin and adipoQ receptor family member 3a <- (fem gon)

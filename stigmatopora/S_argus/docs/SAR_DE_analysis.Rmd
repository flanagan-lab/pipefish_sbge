...
MGonad_trin_genes <- cbind(rownames(head(gonad_con_res_p05[order(
  gonad_con_res_p05$log2FoldChange, decreasing = TRUE),], n=6805)))
...
FGonad_trin_genes <- cbind(rownames(head(gonad_con_res_p05[order(
  gonad_con_res_p05$log2FoldChange, decreasing = FALSE),], n=3554)))
...
blast_output_merged <- lapply(blast_output_filtered, function(dataframe){
  ...
  #browser()
  output <- merge(dataframe, unique(genes[,7:9]), by = "prot_id")
...
```{r merge-cory-blast-logfc}

#blast_output_merged$SA_femG_TRgenes_corythoicthys_blast <- merge(as.data.frame(gill_fem_biased), blast_output_merged$SA_femG_TRgenes_corythoicthys_blast, by = "trin_geneid", all.y = TRUE)

#blast_output_merged$SA_femGon_TRgenes_corythoicthys_blast <- merge(as.data.frame(gonad_fem_biased), blast_output_merged$SA_femGon_TRgenes_corythoicthys_blast, by = "trin_geneid", all.y = TRUE)

#blast_output_merged$SA_femL_TRgenes_corythoicthys_blast <- merge(as.data.frame(liver_fem_biased), blast_output_merged$SA_femL_TRgenes_corythoicthys_blast, by = "trin_geneid", all.y = TRUE)

#blast_output_merged$SA_maleG_TRgenes_corythoicthys_blast <- merge(as.data.frame(gill_mal_biased), blast_output_merged$SA_maleG_TRgenes_corythoicthys_blast, by = "trin_geneid", all.y = TRUE)

#blast_output_merged$SA_maleGon_TRgenes_corythoicthys_blast <- merge(as.data.frame(gonad_mal_biased), blast_output_merged$SA_maleGon_TRgenes_corythoicthys_blast, by = "trin_geneid", all.y = TRUE)

#blast_output_merged$SA_maleL_TRgenes_corythoicthys_blast <- merge(as.data.frame(liver_mal_biased), blast_output_merged$SA_maleL_TRgenes_corythoicthys_blast, by = "trin_geneid", all.y = TRUE)

### things to try
# cory_gff[cory_gff$feature == "CDS" & cory_gff$seqname == "NC_080403.1" & cory_gff$start = 19590472,] <- fuzzy match start and end
```

```{r fuzzy-match}

## This didn't really work, but it could be a me problem
# Convert start and end columns to numeric
#cory_gff$start <- as.numeric(cory_gff$start)
#cory_gff$end <- as.numeric(cory_gff$end)

# Create function to perform fuzzy matching on the `start` and `end` columns
#fuzzy_match <- function(x, y, tolerance) {
 # abs(x - y) <= tolerance
#}

# Apply the fuzzy matching to the start and end columns
#tolerance <- 10000
#result <- cory_gff[
 # fuzzy_match(cory_gff[[start]], cory_gff[[end]], tolerance) &
  #cory_gff$feature == "CDS" &
  #cory_gff$seqname == "NC_080395.1"
#]
...
tolerance <- 1000
result <- cory_gff %>%
  filter(feature == "CDS", seqname == "NC_080395.1") %>%
  mutate(start = as.numeric(start),
         end = as.numeric(end)) %>%
  rowwise() %>%
  mutate(match = abs(start - end) <= tolerance) %>%
  select(-match)

#that also didn't really work, maybe try by phase?


```


```{r write-names, eval=FALSE}
#write once then comment out 

write.table(c(blast_output_merged$SA_femG_TRgenes_corythoicthys_blast$gene_name,
            blast_output_merged$SA_femGon_TRgenes_corythoicthys_blast$gene_name,
            blast_output_merged$SA_femL_TRgenes_corythoicthys_blast$gene_name,
            blast_output_merged$SA_maleG_TRgenes_corythoicthys_blast$gene_name,
            blast_output_merged$SA_maleGon_TRgenes_corythoicthys_blast$gene_name,
            blast_output_merged$SA_maleL_TRgenes_corythoicthys_blast$gene_name),
            'data/SA_cory_names.txt',
            sep = " ",
            quote=FALSE,
            row.names = FALSE,
            col.names = FALSE)
...
blast_output_filtered$SA_femG_TRgenes_corythoicthys_blast <- merge(all_FGill_df, blast_output_filtered$SA_femG_TRgenes_corythoicthys_blast, by = "trin_geneid", all.x = TRUE, all.y = TRUE)
blast_output_filtered$SA_femGon_TRgenes_corythoicthys_blast <- merge(all_FGonad_df, blast_output_filtered$SA_femGon_TRgenes_corythoicthys_blast, by = "trin_geneid", all.x = TRUE, all.y = TRUE)
blast_output_filtered$SA_femL_TRgenes_corythoicthys_blast <- merge(all_FLiver_df, blast_output_filtered$SA_femL_TRgenes_corythoicthys_blast, by = "trin_geneid", all.x = TRUE, all.y = TRUE)
blast_output_filtered$SA_maleG_TRgenes_corythoicthys_blast <- merge(all_MGill_df, blast_output_filtered$SA_maleG_TRgenes_corythoicthys_blast, by = "trin_geneid", all.x = TRUE, all.y = TRUE)
blast_output_filtered$SA_maleGon_TRgenes_corythoicthys_blast <- merge(all_MGonad_df, blast_output_filtered$SA_maleGon_TRgenes_corythoicthys_blast, by = "trin_geneid", all.x = TRUE, all.y = TRUE)
blast_output_filtered$SA_maleL_TRgenes_corythoicthys_blast <- merge(all_MLiver_df, blast_output_filtered$SA_maleL_TRgenes_corythoicthys_blast, by = "trin_geneid", all.x = TRUE, all.y = TRUE)
```

```{r export-datasets}
#write.csv(blast_output_filtered$SA_femG_TRgenes_corythoicthys_blast, "data/SA_BLAST_results_femG.csv", row.names = FALSE)
#write.csv(blast_output_filtered$SA_malG_TRgenes_corythoicthys_blast, "data/SA_BLAST_results_malG.csv", row.names = FALSE)

#write.csv(blast_output_filtered$SA_femGon_TRgenes_corythoicthys_blast, "data/SA_BLAST_results_femGon.csv", row.names = FALSE)
#write.csv(blast_output_filtered$SA_malGon_TRgenes_corythoicthys_blast, "data/SA_BLAST_results_malGon.csv", row.names = FALSE)

#write.csv(blast_output_filtered$SA_femL_TRgenes_corythoicthys_blast, "data/SA_BLAST_results_femL.csv", row.names = FALSE)
#write.csv(blast_output_filtered$SA_malL_TRgenes_corythoicthys_blast, "data/SA_BLAST_results_malL.csv", row.names = FALSE)
```

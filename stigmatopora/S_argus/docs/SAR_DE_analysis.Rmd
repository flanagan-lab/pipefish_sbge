...
#remove S39
dds_SA <- dds_SA[, !(dds_SA$ID %in% c("S39", "S19"))]
...
samples <- samples[!(samples$ID %in% c("S39", "S19")),]
df <- as.data.frame(samples[,c("sex", "organ")])
rownames(df) <- samples$ID
df$organ <- as.character(df$organ)
...
text(pca_plotting_data$PC1[pca_plotting_data$ID == "S39", "S19"] + 50,
     pca_plotting_data$PC4[pca_plotting_data$ID == "S39", "S19"] - 25,
     labels = pca_plotting_data$ID[pca_plotting_data$ID == "S39", "S19"])
...
ddsMF_SA <- ddsMF_SA[, !(ddsMF_SA$ID %in% c("S19", "S39"))]
...
MLiver_trin_genes <- cbind(rownames(head(liver_con_res_p05[order(
  liver_con_res_p05$log2FoldChange, decreasing = TRUE),], n=104)))
#Run once to create the file and then commented out
...
Fliver_trin_genes <- cbind(rownames(head(liver_con_res_p05[order(
  liver_con_res_p05$log2FoldChange, decreasing = FALSE),], n=494)))
...
MGill_trin_genes <- cbind(rownames(head(gill_con_res_p05[order(
  gill_con_res_p05$log2FoldChange, decreasing = TRUE),], n=19)))
...
FGill_trin_genes <- cbind(rownames(head(gill_con_res_p05[order(
  gill_con_res_p05$log2FoldChange, decreasing = FALSE),], n=18)))
...
MGonad_trin_genes <- cbind(rownames(head(gonad_con_res_p05[order(
  gonad_con_res_p05$log2FoldChange, decreasing = TRUE),], n=1197)))
...
FGonad_trin_genes <- cbind(rownames(head(gonad_con_res_p05[order(
  gonad_con_res_p05$log2FoldChange, decreasing = FALSE),], n=5166)))
...
From the MA plots we can see the highest number of biased genes are found in the gonads and then the liver and lastly the gills. In both the liver and the gonads there are more male-biased genes than female-biased genes.
...
dds_SA_exp$organ <- ifelse(dds_SA_exp$organ %in% c("ovaries", "testis"),
                           paste0("gonad"), 
                           paste0(dds_SA_exp$organ))
...
for (organ in organs) {
  ...
  #browser()
  tmp <- as.data.frame(counts(dds_SA_exp, normalized = TRUE)[, dds_SA_exp$organ == organ])
  tmpF <- as.data.frame(counts(dds_SA_exp, normalized = TRUE)[, dds_SA_exp$organ == organ & dds_SA_exp$sex == "F"])
  tmpM <- as.data.frame(counts(dds_SA_exp, normalized = TRUE)[, dds_SA_exp$organ == organ & dds_SA_exp$sex == "M"])
...
organs <- unique(colData(ddsMF_SA_exp_filtered)$organ)
...
sex_specific_counts <- data.frame(tissue = rep(c("Gill", "Gonad", "Liver"), 2),
                                  sex = c(rep("M", 3),
                                          rep("F", 3)),
                                  num_genes = c(nrow(SA_sex_specific_genes$gill_male_specific),
                                                nrow(SA_sex_specific_genes$gonad_male_specific),
                                                nrow(SA_sex_specific_genes$liver_male_specific),
                                                nrow(SA_sex_specific_genes$gill_female_specific),
                                                nrow(SA_sex_specific_genes$gonad_female_specific),
                                                nrow(SA_sex_specific_genes$liver_female_specific))
                                  )
...
In the **Male Gills** there are `r nrow(SA_sex_specific_genes$Gill_male_specific[rownames(SA_sex_specific_genes$Gill_male_specific) %in% rownames(gill_mal_biased),])` genes that overlap between sex-biased and sex-specific, there are `r nrow(SA_sex_specific_genes$Gonad_male_specific[rownames(SA_sex_specific_genes$Gonad_male_specific) %in% rownames(gonad_mal_biased),])` overlapping in the **gonads** and `r nrow(SA_sex_specific_genes$Liver_male_specific[rownames(SA_sex_specific_genes$liver_male_specific) %in% rownames(liver_mal_biased),])` overlapping in the **liver**. For females we have `r nrow(SA_sex_specific_genes$Gill_female_specific[rownames(SA_sex_specific_genes$Gill_female_specific) %in% rownames(gill_fem_biased),])` overlapping sex-specific and sex-biased genes in the **gills**, `r nrow(SA_sex_specific_genes$Gonad_female_specific[rownames(SA_sex_specific_genes$Gonad_female_specific) %in% rownames(gonad_fem_biased),])` genes in the **gonads**, and `r nrow(SA_sex_specific_genes$liver_female_specific[rownames(SA_sex_specific_genes$liver_female_specific) %in% rownames(liver_fem_biased),])` genes in the **liver**.

```{r gills-sex-specific, eval = FALSE}

## Makes sure factors are factors
ddsMF_SA$organ <- factor(ddsMF_SA$organ, levels=c("gill","gonad","liver"))
ddsMF_SA$sex <- factor(ddsMF_SA$sex, levels=c("F","M"))

## Female sex-specific
fem_sp_geneIDs <- rownames(SA_sex_specific_genes$gill_female_specific)
...
for(gene in fem_sp_geneIDs){
  ...
  plotCounts(ddsMF_SA[,ddsMF_SA$organ == 'gill'], gene=gene, intgroup="sex")
...
for(gene in gill_fem_biased_geneIDS){
  ...
  plotCounts(ddsMF_SA[,ddsMF_SA$organ == 'gill'], gene=gene, intgroup="sex", col= 'salmon', pch = 19)
...
```{r}
#Run once to create the file and then commented out
#write.table(cbind(rownames(SA_sex_specific_genes$gill_male_specific)),
 #           'SA_malG_specific_TRgenes.txt', 
  #          sep = "", 
   #         quote=FALSE, 
    #        row.names = FALSE, 
     #       col.names = FALSE)
#write.table(cbind(rownames(SA_sex_specific_genes$gill_female_specific)),
 #           'SA_femG_specific_TRgenes.txt', 
  #          sep = "", 
   #         quote=FALSE, 
    #        row.names = FALSE, 
     #       col.names = FALSE)
#write.table(cbind(rownames(SA_sex_specific_genes$gonad_male_specific)),
 #           'SA_malGon_specific_TRgenes.txt', 
  #          sep = "", 
   #         quote=FALSE, 
    #        row.names = FALSE, 
     #       col.names = FALSE)
#write.table(cbind(rownames(SA_sex_specific_genes$gonad_female_specific)),
 #           'SA_femGon_specific_TRgenes.txt', 
  #          sep = "", 
   #         quote=FALSE, 
    #        row.names = FALSE, 
     #       col.names = FALSE)
#write.table(cbind(rownames(SA_sex_specific_genes$liver_male_specific)),
 #           'SA_malL_specific_TRgenes.txt', 
  #          sep = "", 
   #         quote=FALSE, 
    #        row.names = FALSE, 
     #       col.names = FALSE)
#write.table(cbind(rownames(SA_sex_specific_genes$liver_female_specific)),
 #           'SA_femL_specific_TRgenes.txt', 
  #          sep = "", 
   #         quote=FALSE, 
    #        row.names = FALSE, 
     #       col.names = FALSE)
...
makeblastdb -in /home/rccuser/shared/emily_files/S_argus_2024/c_intestinalis_data/data/GCF_030265065.1/GCF_030265065.1_ASM3026506v1_genomic.fna -out c_intestinalis_genome -dbtype nucl
```
...
This script was run as `bash ../scripts/blast_pipeline.sh TRgenes/ ../scripts/subset_fasta_file super_trinity_genes.fasta c_intestinalis_genome/c_intestinalis_genome TRgenes`
...
SA_blast_files <- list.files(blast_path, pattern = "cory")

#Create an empty list to store my datasets in
SA_blast_list <- list()

#Create a loop to read in all of the blast results
for(file_name in SA_blast_files){
  ...
  SA_blast_list[[blast_name]] <- file_data
}
...
blast_output_filtered <- lapply(SA_blast_list, function(data_frame){
...
```{r export-products}
merged_SAR_femG <- blast_output_merged$SA_femG_TRgenes_corythoicthys_blast
#write.csv(merged_SAR_femG, 'data/blast_output_merged_femG.csv')

merged_SAR_femGon <- blast_output_merged$SA_femGon_TRgenes_corythoicthys_blast
#write.csv(merged_SAR_femGon, 'data/blast_output_merged_femGon.csv')

merged_SAR_femL <- blast_output_merged$SA_femL_TRgenes_corythoicthys_blast
#write.csv(merged_SAR_femL, 'data/blast_output_merged_femL.csv')

merged_SAR_malG <- blast_output_merged$SA_malG_TRgenes_corythoicthys_blast
#write.csv(merged_SAR_malG, 'data/blast_output_merged_malG.csv')

merged_SAR_malGon <- blast_output_merged$SA_malGon_TRgenes_corythoicthys_blast
#write.csv(merged_SAR_malGon, 'data/blast_output_merged_malGon.csv')

merged_SAR_malL <- blast_output_merged$SA_malL_TRgenes_corythoicthys_blast
#write.csv(merged_SAR_malL, 'data/blast_output_merged_malL.csv')
```
...
#Getting all the diff. expressed genes in males
...
blast_output_filtered$SA_femG_TRgenes_blast <- merge(all_FGill_df, blast_output_filtered$SA_femG_TRgenes_corythoicthys_blast, by = "trin_geneid", all.x = TRUE, all.y = TRUE)

blast_output_filtered$SA_femGon_TRgenes_blast <- merge(all_FGonad_df, blast_output_filtered$SA_femGon_TRgenes_corythoicthys_blast, by = "trin_geneid", all.x = TRUE, all.y = TRUE)

blast_output_filtered$SA_femL_TRgenes_blast <- merge(all_FGonad_df, blast_output_filtered$SA_femL_TRgenes_corythoicthys_blast, by = "trin_geneid", all.x = TRUE, all.y = TRUE)

blast_output_filtered$SA_maleG_TRgenes_blast <- merge(all_MGill_df, blast_output_filtered$SA_maleG_TRgenes_corythoicthys_blast, by = "trin_geneid", all.x = TRUE, all.y = TRUE)

blast_output_filtered$SA_maleGon_TRgenes_blast <- merge(all_MGonad_df, blast_output_filtered$SA_maleGon_TRgenes_corythoicthys_blast, by = "trin_geneid", all.x = TRUE, all.y = TRUE)

blast_output_filtered$SA_maleL_TRgenes_blast <- merge(all_MLiver_df, blast_output_filtered$SA_maleL_TRgenes_corythoicthys_blast, by = "trin_geneid", all.x = TRUE, all.y = TRUE)
```

```{r export-datasets}
#BLAST results gills
#write.csv(blast_output_filtered$SA_femG_TRgenes_blast, "data/SA_BLAST_results_femG.csv", row.names = FALSE)
#write.csv(blast_output_filtered$SA_maleG_TRgenes_blast, "data/SA_BLAST_results_malG.csv", row.names = FALSE)

#BLAST results gonads
#write.csv(blast_output_filtered$SA_femGon_TRgenes_blast, "data/SA_BLAST_results_femGon.csv", row.names = FALSE)
#write.csv(blast_output_filtered$SA_maleGon_TRgenes_blast, "data/SA_BLAST_results_malGon.csv", row.names = FALSE)

#BLAST results liver
#write.csv(blast_output_filtered$SA_femL_TRgenes_blast, "data/SA_BLAST_results_femL.csv", row.names = FALSE)
#write.csv(blast_output_filtered$SA_maleL_TRgenes_blast, "data/SA_BLAST_results_malL.csv", row.names = FALSE)
```


### Blasting the sex-specific genes against the _C. intestinalis_ genome
The trinity genes that were pulled out for the sex-specific genes above were blasted with `blastn` against the _Corythoicthys intestinalis_ genome in the RCC. I then need to load those BLAST results into R, filter them, and pull out what the _C. intestinalis_ gene names are that correspond to them. 

First, I read in all of the BLAST results for the sex-specific genes in each organ.

```{r readin_blast_results_SS}
#Specify the directory where BLAST results are located
blast_path <- "data/cory_BLAST/specific/"

#Create a list of the files I want
SA_blast_files <- list.files(blast_path, pattern = "specific")

#Create an empty list to store my datasets in
SA_blast_list_sex_specific <- list()
...
for(file_name in SA_blast_files){
  ...
  # Create a new object with a name based on the file name
  blast_name <- sub(".txt$", "", file_name) #Removes the file extension
  SA_blast_list_sex_specific[[blast_name]] <- file_data
}
...
```{r filter-BLAST-SS}
#Use lapply to apply the function to each dataset stored in the list created above
blast_output_filtered_SS <- lapply(SA_blast_list_sex_specific, function(data_frame){
  ...
  for(gene in uniqueID$trin_geneid){
    ...
    # check that there is a single gene ID in intestinalis, if not, only first row is kept
...
```{r long_BLAST, eval=FALSE}
#Create a long format dataset that has the tissue information, BLAST gene and protein, geneID, and sex 
blast_long_SS <- data.frame(
  tissue=c(rep("gill", 
               sum(nrow(blast_output_filtered_SS$SA_femG_specific_TRgenes_corythoicthys_blast),
                   nrow(blast_output_filtered_SS$SA_malG_specific_TRgenes_corythoicthys_blast))),
           rep("gonad", 
               sum(nrow(blast_output_filtered_SS$SA_femGon_specific_TRgenes_corythoicthys_blast),
                   nrow(blast_output_filtered_SS$SA_malGon_specific_TRgenes_corythoicthys_blast))),
           rep("liver",
               sum(nrow(blast_output_filtered_SS$SA_femL_specific_TRgenes_corythoicthys_blast),
                   nrow(blast_output_filtered_SS$SA_malL_specific_TRgenes_corythoicthys_blast)))
         ),
  sex=c(rep("Female",
            nrow(blast_output_filtered_SS$SA_femG_specific_TRgenes_corythoicthys_blast)),
        rep("Male",
            nrow(blast_output_filtered_SS$SA_malG_specific_TRgenes_corythoicthys_blast)),
        rep("Female",
            nrow(blast_output_filtered_SS$SA_femGon_specific_TRgenes_corythoicthys_blast)),
        rep("Male",
            nrow(blast_output_filtered_SS$SA_malGon_specific_TRgenes_corythoicthys_blast)),
        rep("Female",
            nrow(blast_output_filtered_SS$SA_femL_specific_TRgenes_corythoicthys_blast)),
        rep("Male",
            nrow(blast_output_filtered_SS$SA_malL_specific_TRgenes_corythoicthys_blast))
        ),
  gene=c(blast_output_filtered_SS$SA_femG_specific_TRgenes_corythoicthys_blast$gene,
         blast_output_filtered_SS$SA_malG_specific_TRgenes_corythoicthys_blast$gene,
         blast_output_filtered_SS$SA_femGon_specific_TRgenes_corythoicthys_blast$gene,
         blast_output_filtered_SS$SA_malGon_specific_TRgenes_corythoicthys_blast$gene,
         blast_output_filtered_SS$SA_femL_specific_TRgenes_corythoicthys_blast$gene,
         blast_output_filtered_SS$SA_malL_specific_TRgenes_corythoicthys_blast$gene
         ),
  protein=c(blast_output_filtered_SS$SA_femG_specific_TRgenes_corythoicthys_blast$protein,
            blast_output_filtered_SS$SA_malG_specific_TRgenes_corythoicthys_blast$protein,
            blast_output_filtered_SS$SA_femGon_specific_TRgenes_corythoicthys_blast$protein,
            blast_output_filtered_SS$SA_malGon_specific_TRgenes_corythoicthys_blast$protein,
            blast_output_filtered_SS$SA_femL_specific_TRgenes_corythoicthys_blast$protein,
            blast_output_filtered_SS$SA_malL_specific_TRgenes_corythoicthys_blast$protein
            ),
  trinityID=c(blast_output_filtered_SS$SA_femG_specific_TRgenes_corythoicthys_blast$trin_geneid,
              blast_output_filtered_SS$SA_malG_specific_TRgenes_corythoicthys_blast$trin_geneid,
              blast_output_filtered_SS$SA_femGon_specific_TRgenes_corythoicthys_blast$trin_geneid,
              blast_output_filtered_SS$SA_malGon_specific_TRgenes_corythoicthys_blast$trin_geneid,
              blast_output_filtered_SS$SA_femL_specific_TRgenes_corythoicthys_blast$trin_geneid,
              blast_output_filtered_SS$SA_malL_specific_TRgenes_corythoicthys_blast$trin_geneid)
  )

#write.csv(blast_long_SS, "data/SA_BLAST_results_sex_specific.csv", row.names = FALSE)
```
...
#Create a list of the files I want
SA_blast_files <- list.files(blast_path, pattern = "blast")
...
blast_output_merged$SA_femG_TRgenes_prot_blast <- merge(as.data.frame(gill_fem_biased), blast_output_merged$SA_femG_TRgenes_prot_blast, by = "trin_geneid", all.y = TRUE)

blast_output_merged$SA_femGon_TRgenes_prot_blast <- merge(as.data.frame(gonad_fem_biased), blast_output_merged$SA_femGon_TRgenes_prot_blast, by = "trin_geneid", all.y = TRUE)

blast_output_merged$SA_femL_TRgenes_prot_blast <- merge(as.data.frame(liver_fem_biased), blast_output_merged$SA_femL_TRgenes_prot_blast, by = "trin_geneid", all.y = TRUE)

blast_output_merged$SA_maleG_TRgenes_prot_blast <- merge(as.data.frame(gill_mal_biased), blast_output_merged$SA_maleG_TRgenes_prot_blast, by = "trin_geneid", all.y = TRUE)

blast_output_merged$SA_maleGon_TRgenes_prot_blast <- merge(as.data.frame(gonad_mal_biased), blast_output_merged$SA_maleGon_TRgenes_prot_blast, by = "trin_geneid", all.y = TRUE)

blast_output_merged$SA_maleL_TRgenes_prot_blast <- merge(as.data.frame(liver_mal_biased), blast_output_merged$SA_maleL_TRgenes_prot_blast, by = "trin_geneid", all.y = TRUE)
```

```{r, eval=FALSE}
#write once and then comment out 

#write.table(c(blast_output_merged$SA_femG_TRgenes_prot_blast$gene_name,
 #             blast_output_merged$SA_femGon_TRgenes_prot_blast$gene_name,
  #            blast_output_merged$SA_femL_TRgenes_prot_blast$gene_name,
   #           blast_output_merged$SA_maleG_TRgenes_prot_blast$gene_name,
    #          blast_output_merged$SA_maleGon_TRgenes_prot_blast$gene_name,
     #         blast_output_merged$SA_maleL_TRgenes_prot_blast$gene_name),
      #      'data/SA_GOnames.txt',
       #      sep = " ", 
        #     quote=FALSE, 
         #    row.names = FALSE, 
...
#write.table(rbind(blast_output_merged$SA_femG_TRgenes_prot_blast[,c("gene_name", "log2FoldChange")],
 #                 blast_output_merged$SA_femGon_TRgenes_prot_blast[,c("gene_name", "log2FoldChange")],
  #                blast_output_merged$SA_femL_TRgenes_prot_blast[,c("gene_name", "log2FoldChange")],
   #               blast_output_merged$SA_malG_TRgenes_prot_blast[,c("gene_name", "log2FoldChange")],
    #              blast_output_merged$SA_malGon_TRgenes_prot_blast[,c("gene_name", "log2FoldChange")],
     #             blast_output_merged$SA_malL_TRgenes_prot_blast[,c("gene_name", "log2FoldChange")]),
      #      'data/SA_GOnames_FC.txt',
       #     sep = " ",
        #    quote = FALSE,
         #   row.names = FALSE,
          #  col.names = FALSE)
...
blast_output_merged$SA_femG_TRgenes_prot_blast <-
  merge(blast_output_merged$SA_femG_TRgenes_prot_blast,
        panther[, c(2,5)], 
        by.x = "gene_name", 
        by.y = "MappedID", 
        all.x = TRUE)
blast_output_merged$SA_femGon_TRgenes_prot_blast <-
  merge(blast_output_merged$SA_femGon_TRgenes_prot_blast,
        panther[, c(2,5)], 
        by.x = "gene_name", 
        by.y = "MappedID", 
        all.x = TRUE)
blast_output_merged$SA_femL_TRgenes_prot_blast <-
  merge(blast_output_merged$SA_femL_TRgenes_prot_blast,
        panther[, c(2,5)], 
        by.x = "gene_name", 
        by.y = "MappedID", 
        all.x = TRUE)
blast_output_merged$SA_maleG_TRgenes_prot_blast <-
  merge(blast_output_merged$SA_maleG_TRgenes_prot_blast,
        panther[, c(2,5)], 
        by.x = "gene_name", 
        by.y = "MappedID", 
        all.x = TRUE)
blast_output_merged$SA_maleGon_TRgenes_prot_blast <-
  merge(blast_output_merged$SA_maleGon_TRgenes_prot_blast,
        panther[, c(2,5)], 
        by.x = "gene_name", 
        by.y = "MappedID", 
        all.x = TRUE)
blast_output_merged$SA_maleL_TRgenes_prot_blast <-
  merge(blast_output_merged$SA_maleL_TRgenes_prot_blast,
        panther[, c(2,5)], 
        by.x = "gene_name", 
        by.y = "MappedID", 
        all.x = TRUE)
...
colnames(all_go_dat) <- c(
  colnames(go_tabs$SA_femG_TRgenes_prot_blast),
  "bias_cat", "tissue"
)
...
data_plot <- all_go_sums %>%
  select(tissue, prot_class, prop)

# Create the dot plot
ggplot(data_plot, aes(x = tissue, y = prot_class, size = prop, fill = tissue)) +
  geom_point(alpha = 0.7) +
  scale_fill_manual(values = my_colors) +
  scale_size_continuous(range = c(1, 15)) +
  labs(title = "Tissue Distribution of Proteins",
       x = "Tissue",
       y = "Protein Class",
       size = "Proportion",
       fill = "Tissue Color") +
...
What does it look like if we use the proportion of SBG in each organ?
```{r GO-dotplot-prop-organs}
# Import .csv with proportion information filtered to combine all protein classes that have 2 or fewer hits together per organ
all_go_sums_prop <- read.csv("data/all_go_sums_other.csv")

# Define custom color palette
my_colors <- c("Gonad" = "#EEB422", "Liver" = "#EE8262", "Gill" = "#20B2AA")

# Ensure all tissues are included in the color palette
all_tissues <- unique(all_go_sums_prop$tissue)
if (!all(all_tissues %in% names(my_colors))) {
  warning(paste("Some tissues in the data are not included in the color palette:", setdiff(all_tissues, names(my_colors))))
}

# Select the relevant columns
data_plot <- all_go_sums_prop %>%
  select(tissue, prot_class, prop_organ_SBG)

# Create the dot plot
ggplot(data_plot, aes(x = tissue, y = prot_class, size = prop_organ_SBG, color = tissue)) +
  geom_point(alpha = 0.7) +
  scale_color_manual(values = my_colors) +
  scale_size_continuous(range = c(1, 15)) +
  labs(title = "Tissue Distribution of Proteins",
       x = "Tissue",
       y = "Protein Class",
       size = "Proportion of SBG in Organ",
       color = "Tissue Color") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
#write.table(blast_output_merged$SA_femGon_TRgenes_prot_blast$gene_name,
 #           'data/SA_GOnames_SBG_ovary.txt',
  #          sep = " ",
   #         quote=FALSE,
    #        row.names = FALSE,
     #       col.names = FALSE)

#write.table(blast_output_merged$SA_maleGon_TRgenes_prot_blast$gene_name,
 #           'data/SA_GOnames_SBG_testes.txt',
  #          sep = " ",
   #         quote=FALSE,
    #        row.names = FALSE,
     #       col.names = FALSE)

#write.table(blast_output_merged$SA_femG_TRgenes_prot_blast$gene_name,
 #           'data/SA_GOnames_SBG_gill.txt',
  #          sep = " ",
   #         quote=FALSE,
    #        row.names = FALSE,
     #       col.names = FALSE)

#write.table(blast_output_merged$SA_maleG_TRgenes_prot_blast$gene_name,
 #           'data/SA_GOnames_SBG_gill.txt',
  #          sep = " ",
   #         quote=FALSE,
    #        row.names = FALSE,
...
#write.table(blast_output_merged$SA_femL_TRgenes_prot_blast$gene_name,
 #           'data/SA_GOnames_SBG_liver.txt',
  #          sep = " ",
   #         quote=FALSE,
    #        row.names = FALSE,
     #       col.names = FALSE)

#write.table(blast_output_merged$SA_maleL_TRgenes_prot_blast$gene_name,
 #           'data/SA_GOnames_SBG_liver.txt',
  #          sep = " ",
   #         quote=FALSE,
    #        row.names = FALSE,

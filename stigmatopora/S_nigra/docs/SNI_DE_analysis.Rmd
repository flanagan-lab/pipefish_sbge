...
text(pca_plotting_data$PC1[pca_plotting_data$ID == "S34", "S41"] + 50,
     pca_plotting_data$PC4[pca_plotting_data$ID == "S34", "S41"] - 25,
     labels = pca_plotting_data$ID[pca_plotting_data$ID == "S34", "S41"])
...
listInputall <- list("Female Gill" = rownames(gill_fem_biased),
                  "Male Gill"=rownames(gill_mal_biased), 
                  "Female Liver"=rownames(liver_fem_biased),
                  "Male Liver" = rownames(liver_mal_biased),
                  "Ovaries"=rownames(gonad_fem_biased), 
                  "Testes"=rownames(gonad_mal_biased))

# Define the desired order of sets
desired_order <- c("Female Gill", "Male Gill", "Female Liver", "Male Liver", "Ovaries", "Testes")
...
dds_SN_exp$organ <- ifelse(dds_SN_exp$organ %in% c("ovaries", "testis"),
                           paste0("gonad"), 
                           paste0(dds_SN_exp$organ))

bias_colors <- c("FB" = "#7fc97f", "MB" = "#beaed4", "UB" = "#A9A9A975") 

par(mfrow=c(1,3), oma=c(4,4,2,2), mar=c(1,1,1,0))

for (organ in organs) {
  
  #browser()
  tmp <- as.data.frame(counts(dds_SN_exp, normalized = TRUE)[, dds_SN_exp$organ == organ])
  tmpF <- as.data.frame(counts(dds_SN_exp, normalized = TRUE)[, dds_SN_exp$organ == organ & dds_SN_exp$sex == "F"])
  tmpM <- as.data.frame(counts(dds_SN_exp, normalized = TRUE)[, dds_SN_exp$organ == organ & dds_SN_exp$sex == "M"])
...
par(mfrow=c(1, 2))
barplot(bias_cat_gill[,biased_bins[biased_bins!="Unbiased"]], beside = TRUE, ylim = c(0, max(500) + 1), col = c("#7fc97f", "#beaed4"),
...
#barplot(bias_cat_gonad[,biased_bins[biased_bins!="Unbiased"]], beside = TRUE, ylim = c(0, max(bias_cat_table) + 1), col = c("#7fc97f", "#beaed4"),
 #       main = "Gonad")
barplot(bias_cat_liver[,biased_bins[biased_bins!="Unbiased"]], beside = TRUE, ylim = c(0, max(500) + 1), col = c("#7fc97f", "#beaed4"),
        main = "Liver")
...
# Create the dot plot
ggplot(data_plot, aes(x = tissue, y = prot_class, size = prop_organ_SBG, fill = tissue)) +
  geom_point(alpha = 0.7) +
  scale_fill_manual(values = my_colors) +
  scale_size_continuous(range = c(1, 15)) +
  labs(title = "Tissue Distribution of Proteins",
...
#write.table(blast_output_merged$SN_maleGon_allTRgenes_danio_blast$gene_name,
 #           'data/SN_GOnames_SBG_testes.txt',
  #          sep = " ",
   #         quote=FALSE,
    #        row.names = FALSE,
     #       col.names = FALSE)
...
#write.table(blast_output_merged$SN_femL_allTRgenes_danio_blast$gene_name,
 #           'data/SN_GOnames_SBG_fLiver.txt',
  #          sep = " ",
   #         quote=FALSE,
    #        row.names = FALSE,
     #       col.names = FALSE)
...
#write.table(blast_output_merged$SN_maleL_allTRgenes_danio_blast$gene_name,
 #           'data/SN_GOnames_SBG_mLiver.txt',
  #          sep = " ",
   #         quote=FALSE,
    #        row.names = FALSE,
     #       col.names = FALSE)

#write.table(blast_output_merged$SN_femG_allTRgenes_danio_blast$gene_name,
 #           'data/SN_GOnames_SBG_fgill.txt',
  #          sep = " ",
   #         quote=FALSE,
    #        row.names = FALSE,
     #       col.names = FALSE)

#write.table(blast_output_merged$SN_maleG_allTRgenes_danio_blast$gene_name,
 #           'data/SN_GOnames_SBG_mgill.txt',
  #          sep = " ",
   #         quote=FALSE,
    #        row.names = FALSE,
     #       col.names = FALSE)

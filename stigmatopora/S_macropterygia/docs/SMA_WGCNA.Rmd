...
write.csv(logFC_long_all,"data/logFC_long_all_module_updated.csv",row.names = FALSE)
```

```{r}
logFC_long_all<-read.csv("data/logFC_long_all_module_updated.csv")
...
```{r}
mod_lm<-lm(rank(logFC_long_all$logFC)~as.factor(logFC_long_all$tissue) *as.factor(logFC_long_all$module))
anova(mod_lm)
...
```{r}
module_medians<-tapply(logFC_long_all$logFC,
       paste(logFC_long_all$tissue, logFC_long_all$module),
       median,
       na.rm=TRUE)
...
abline(h=-2,lty=2,lwd=2,col="grey")
vioplot(logFC_long_all$logFC~logFC_long_all$tissue + logFC_long_all$module,
        col=plot_cols,
        add=TRUE,
        bty='n',
        xaxt='n',
        ...
        plotCentre=TRUE)
points(x=13:15,
       y=logFC_long_all$logFC[which(logFC_long_all$module=="grey")],
       col="black",
       bg="grey",
       pch=23,
       cex=1.25)
...
cor_vals<-cor_vals[levels(logFC_long_all$module)]
...
```{r, eval=FALSE}
tau_col_dat<-logFC_long_all[which(logFC_long_all$module != "grey"),]
...
hub_blast_info<-as.data.frame(do.call(rbind, lapply(hub_blast_filtered$sseqid, function(sid,gff){
...
hub_blast_info$V2<-round(as.numeric(hub_blast_info$V2),2)

hub_blast_filtered$GeneID<-hub_blast_info$V2
hub_blast_filtered$product<-hub_blast_info$V1
...
```{r}
go_anns<-apply(hub_blast_filtered, 1, function(this_gene, annotations){
...
hub_blast_filtered$GO<-go_anns

all_go_anns<-do.call(rbind,apply(hub_blast_filtered, 1, function(this_gene, annotations){
  #browser()
  this_ann<-annotations[annotations$GeneID==as.numeric(this_gene["GeneID"]),]
...
unmatched<-all_hub_genes[!all_hub_genes$Gene %in% matched_genes,]

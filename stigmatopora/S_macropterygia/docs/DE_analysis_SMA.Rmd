...
```{r heatmap2, echo=FALSE, fig.cap= "Heatmap showing the expression level across all organs for the top 20 genes with the highest expression levels. S10 excluded."}
#Pull out the top 20 rows with the most expression
select <- order(rowMeans(counts(dds_SM_exp, normalized = TRUE)),
                decreasing = TRUE)[1:20]
df <- as.data.frame(colData(dds_SM)[,c("sex", "organ")]) 
...
```{r heatmap_genes2}
#Pull out the corresponding trinity_geneIDS that are plotted in the heatmap
heatmap_TG_no10 <- cbind(row.names(assay(vsd)[select,]))
#Run once to create the file and then commented out
#write.table(heatmap_TG_no10,
 #           'heatmap_trinitygenes_no10.txt',
  #          sep = "",
   #         quote=FALSE,
    #        row.names = FALSE,
     #       col.names = FALSE)
...
pca_plotting_data$organ <- samples$organ[, samples$ID != "S10"]
pca_plotting_data$sex <- samples$sex[, samples$ID != "S10"]

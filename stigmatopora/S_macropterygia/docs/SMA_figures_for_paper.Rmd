...
logFC_long <- read.csv("data/logFC_tau_module_all.csv",
...
logFC_long$group<-paste0(logFC_long$bias_cat,
                             logFC_long$bias)
logFC_long$group<-factor(logFC_long$group,
                             levels=c(
                               "UnbiasedUB",
                               "LowFB",
                               "MedFB",
...
```{r}
plotting_dat<-logFC_long[!is.na(logFC_long$tau),]
```



```{r}
for(organ in c("Liver","Gonad")){
  cairo_pdf(paste0("figs/Tau_",organ,".pdf"),height=5,width=7)
  par(mar=c(3,5.5,1,1))
  this_dat<-plotting_dat[plotting_dat$tissue==organ,]
  this_dat<-this_dat[!is.na(this_dat$group),]
  r<-ridgeline(x=this_dat$tau,
            y=this_dat$group,
...
```{r}
cairo_pdf(paste0("figs/Tau_","Gill",".pdf"),height=5,width=7)
par(mar=c(3,5.5,1,1))
  this_dat<-plotting_dat[plotting_dat$tissue=="Gill",]
  this_dat<-this_dat[!is.na(this_dat$group),]
  this_dat$group<-factor(this_dat$group,
                             levels=c(
                               "UnbiasedUB",
                               "LowFB",
                               "MedFB",

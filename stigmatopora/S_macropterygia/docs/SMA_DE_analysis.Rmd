...
Sample S23 (Mapping rate = 33.7558%) and S30 (Mapping rate = 11.6745%) both had lower than expected gene counts, so they were filtered out entirely. S10 and S15 were clustering in unexpected ways, so they were also removed from further analyses. 
...
samples <- samples[!(samples$ID %in% c("S10", "S15","S23", "S30")),]
...
##Remove S10, S15, S23, and S30 from the dataset
...
all_MLiver_trin_genes <- cbind(rownames(head(liver_con_res_p05[order(
  liver_con_res_p05$log2FoldChange, decreasing = TRUE),], n=322)))
#Run once to create the file and then commented out
...
all_Fliver_trin_genes <- cbind(rownames(head(liver_con_res_p05[order(
  liver_con_res_p05$log2FoldChange, decreasing = FALSE),], n=59)))
...
logFC_long_all$bias_cat <- ifelse(logFC_long_all$bias == "UB", 
                              ...
                              ifelse(abs(logFC_long_all$logFC) >= 2 &
                                     ...
                                     ifelse(abs(logFC_long_all$logFC) >= 3 &
                                              abs(logFC_long_all$logFC) < 4,
                                            biased_bins[3],
                                            ifelse(abs(logFC_long_all$logFC) >= 4 &
                                                     abs(logFC_long_all$logFC) < 8,
                                                   biased_bins[4],
                                                   biased_bins[5])
                                            )
                                     )
...
logFC_long$bias_cat <- ifelse(logFC_long$bias == "UB",
                              ...
                              ifelse(abs(logFC_long$logFC) >= 2 & abs(logFC_long$logFC) < 3,
                                     biased_bins[2],
                                     ifelse(abs(logFC_long$logFC) >= 3 & abs(logFC_long$logFC) < 4,
                                            biased_bins[3],
...
```{r get-median-expression}
logFC_log_cats <- data.frame(matrix(ncol = ncol(logFC_long) + 1,
                                    nrow = 0))
colnames(logFC_log_cats) <- c(colnames(logFC_long), "bias_cats")

#Make a vector that contains all of the groupings
biased_bins <- c("Unbiased", "Low", "Med", "High", "Extreme", "Sex-specific")

#browser()
for (organ in unique(logFC_long$tissue)) {
  
  #Subset dataset for one organ
  tmp <- logFC_long[logFC_long$tissue == organ, ]
  
  #Calculate the quartiles of expression for that organ
  quants <- quantile(abs(tmp$logFC[tmp$bias != "UB"]),
                     prob = c(0.25, 0.5, 0.75),
                     type = 1,
                     na.rm = TRUE)
  
  tmp$bias_cat <- ifelse(tmp$bias == "UB",
                                biased_bins[1],
                                ifelse(abs(tmp$logFC) < quants[[1]],
                                       biased_bins[2],
                                       ifelse(abs(tmp$logFC) >= quants[[1]] & abs(tmp$logFC) < quants[[2]],
                                              biased_bins[3],
                                              ifelse(abs(tmp$logFC) >= quants[[2]] & abs(tmp$logFC) < quants[[3]],
                                                     biased_bins[4],
                                                     biased_bins[[5]]
                                                     )
                                              )
  ...
  #Merge datatsets
  logFC_log_cats <- rbind(logFC_log_cats, tmp)
...
for(i in 1:length(SM_sex_specific_genes)){
  ...
  for(j in 1:nrow(tmp)){
    ...
    if(one_gene[["geneID"]] %in%
       logFC_log_cats[logFC_log_cats$tissue == organs[[i]] &
                  logFC_log_cats$bias == bias[[i]],"geneID"]){
       
      logFC_log_cats[logFC_log_cats$geneID == one_gene[["geneID"]] &
                   logFC_log_cats$tissue == organs[[i]] &
                   logFC_log_cats$bias == bias[[i]],
                 "bias_cat"] <- "Sex-specific"
    }else{
      
      one_gene_dat <- data.frame(matrix(ncol= ncol(logFC_log_cats),
                                        nrow=1))
      colnames(one_gene_dat) <- colnames(logFC_log_cats)
      ...
      logFC_log_cats <- rbind(one_gene_dat, logFC_log_cats)
      
      rownames(logFC_log_cats) <- NULL
    }
  }
}

#Create  subset of our long dataset that does not include the non-biased genes
logFC_long_noNB <- logFC_log_cats[logFC_log_cats$bias_cat != "Unbiased",]
...
par(mfrow=c(1, 3))
barplot(bias_cat_gill[,biased_bins[biased_bins!="Unbiased"]], beside = TRUE, ylim = c(0, 40 + 1), col = c("#7fc97f", "#beaed4"),
...
barplot(bias_cat_gonad[,biased_bins[biased_bins!="Unbiased"]], beside = TRUE, ylim = c(0, max(bias_cat_table) + 1), col = c("#7fc97f", "#beaed4"),
        main = "Gonad")
barplot(bias_cat_liver[,biased_bins[biased_bins!="Unbiased"]], beside = TRUE, ylim = c(0, 150 + 1), col = c("#7fc97f", "#beaed4"),
        main = "Liver")

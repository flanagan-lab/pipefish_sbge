...
# get the sex biased ones
logFC_long_tauSB <- read.csv("data/logFC_long_taubiasSB.csv")


blast_SB_merged <- merge(logFC_long_tauSB, all_blast_filtered, by.x = "geneID", by.y="qseqid", all = TRUE)

#add sex specific dataset
logFC_long_tauSS <- read.csv("data/logFC_long_taubias_SS.csv")


blast_SB_SS_merged <- merge(blast_SB_merged, 
                      logFC_long_tauSS[which(logFC_long_tauSS$bias_cat=="Sex-specific"),], 
                            by = "geneID", 
                            all = TRUE)

blast_SB_SS_merged$bias_cat.x[is.na(blast_SB_SS_merged$bias_cat.x)]<-blast_SB_SS_merged$bias_cat.y[is.na(blast_SB_SS_merged$bias_cat.x)]
#write once then comment out
write.csv(as.data.frame(blast_SB_SS_merged), "data/SM_blast_ann_SB_SS_merged.csv", row.names = TRUE)
```

...
# OrthoFinder output: columns are Orthogroup, then species columns
orthogroup_long <- orthogroups %>%
  pivot_longer(-Orthogroup, names_to = "Species", values_to = "GeneList") %>%
  separate_rows(GeneList, sep = ", ") %>%
  filter(!is.na(GeneList))  # remove empty rows

# Filter for NZ and AUS species genes only
orthogroup_nz <- orthogroup_long %>% filter(Species == "NZ_Snigra")
orthogroup_aus <- orthogroup_long %>% filter(Species == "AUS_Snigra")

#Get intersecting orthogroups
common_orthogroups <- intersect(orthogroup_nz$Orthogroup, orthogroup_aus$Orthogroup)

# Check to make sure there are common orthogroups
print(length(common_orthogroups)) # there are no common orthogroups

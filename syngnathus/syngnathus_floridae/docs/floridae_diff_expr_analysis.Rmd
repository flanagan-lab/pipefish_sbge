...
colnames(cat_meds) <- c("Gill_F","Gill_M",
                        "Liver_F","Liver_M",
                        "Gonad_F","Gonad_M")

cat_meds <- as.data.frame(cat_meds)

quantile(cat_meds$Gill_F, prob = c(0.25, 0.5, 0.75), type = 1)

nrow(unique(cat_meds[rownames(cat_meds[cat_meds$Gill_F < 3.085522, ]) %in% rownames(gill_fem_biased),])) #24

nrow(cat_meds[rownames(cat_meds[cat_meds$Gill_F < 3.085522, ]) %in% rownames(gill_non_biased),]) #15934

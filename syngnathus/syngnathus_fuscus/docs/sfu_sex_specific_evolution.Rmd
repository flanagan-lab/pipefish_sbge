...
```{bash,eval=FALSE, file='../fix_names.sh'}
...
orthogroups_selection_all<-do.call(rbind,apply(orthogroups_selection_all,
                            ...
                            function(gene, expr_info){
                             ...
                             } else if(nrow(this_info) <3){
                               this_gene$tau <- NA
                               this_gene$Gill_logFC<-NA
                               this_gene$Gonad_logFC<-NA
                               this_gene$Liver_logFC<-NA
                             } else{
                               this_gene$tau<-mean(this_info$tau)
                              
                              this_gene$Gill_logFC<-this_info$logFC[this_info$tissue=="Gill"]
                               this_gene$Gonad_logFC<-this_info$logFC[this_info$tissue=="Gonad"]
                               this_gene$Liver_logFC<-this_info$logFC[this_info$tissue=="Liver"]
                              
                             }
                             return(this_gene)
                            }, expr_info=logFC_tau))

# convert the categories if they were
orthogroups_selection_all$DEG_cat[is.na(orthogroups_selection_all$tau)]<-NA
...
We can use a proportion test to see if there is a higher proportion of female-biased genes than unbiased...or chi-squared test?

```{r}
(selchi<-chisq.test(ns, simulate.p.value = TRUE)) # use MC sims
selchi$expected

# to delete -- chisq is better
prop.test(x=ns[1,2], # successes
          n=sum(ns[1,]), # trials
          p=ns[3,2]/(ns[3,1]+ns[3,2]) # probability
)
prop.test(x=ns[2,2], # successes
          n=sum(ns[2,]), # trials
          p=ns[3,2]/(ns[3,1]+ns[3,2]) # probability
)
```
...
```{r fig5, fig.height=3,fig.width=6,dpi=300,fig.format='pdf'}
...
```{r}
orthogroups_selection$plotting_cat<-paste0(orthogroups_selection$sex, "_",orthogroups_selection$DEG_cat)
orthogroups_selection$plotting_cat[orthogroups_selection$plotting_cat=="unbiased_unbiased"]<-"unbiased"
orthogroups_selection$plotting_cat<-paste0(orthogroups_selection$selection,"_",orthogroups_selection$plotting_cat)
counts<-table(orthogroups_selection$plotting_cat)

par(mfrow=c(1,1), mar=c(4,3,2,1))
vioplot(orthogroups_selection$omega ~ orthogroups_selection$plotting_cat,
        xlab="",
         ylab="",
        main="",
        col=rep(c(sex_bias_colors[1],sex_bias_colors[1],sex_bias_colors[2],sex_bias_colors[2],sex_bias_colors[3]),2),
        cex.axis=1,
        cex=1.5,
        frame.plot=FALSE,
        xaxt='n',
        pchMed=21,
        colMed2=rep(c(sex_bias_colors[1],sex_bias_colors[1],sex_bias_colors[2],sex_bias_colors[2],sex_bias_colors[3]),2),
        colMed="black",
        las=1)

plot_labs<-paste0(rep(c("FB","FS","MB","MS","UB"),2),"\nn=",counts)
axis(1,pos=0,at=0:10,labels=c("",
                             plot_labs),tick=FALSE)
lines(x=c(1,5),y=c(8,8))
text(x=3,y=8.5,"no selection")
lines(x=c(6,10),y=c(8,8))
text(x=8,y=8.5,"selection")
...
```{r}
par(mfrow=c(1,3))
hist(orthogroups_selection$omega[orthogroups_selection$sex=="mal" & orthogroups_selection$selection==1],
     xlim=c(0,15),
     main="",
     xlab="",
     ylab="", 
     breaks=seq(0,20,0.5))
hist(orthogroups_selection$omega[orthogroups_selection$sex=="unbiased" & orthogroups_selection$selection==1],
     xlim=c(0,15),
     main="",
     xlab="",
     ylab="", 
     breaks=seq(0,20,0.5))
hist(orthogroups_selection$omega[orthogroups_selection$sex=="fem" & orthogroups_selection$selection==1],
     xlim=c(0,15),
     main="",
     xlab="",
     ylab="",
...
```{r}
keggs<-read.table("08_sex_specific_selection/MM_at20c6re.emapper.annotations.tsv",
                  sep='\t',
                  comment.char="#")
...
How many of the single copy orthogroups were IDed in the KEGG analysis?

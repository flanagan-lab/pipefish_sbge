...
absrel_files<-list.files(pattern="absrel",
                         path = "Single_Copy_Orthologue_absrel",
                         full.names = TRUE) 
...
test_results<-do.call(rbind,lapply(absrel_files, function(file){
  ...
  og<-gsub("Single_Copy_Orthologue_absrel/(OG\\d+)\\.absrel","\\1",file)
...
```{r, eval=FALSE}
orthogroups_selection<-merge(test_results, deg_sc_orthologs, by.x="orthogroup",by.y="Orthogroup")
write.csv(orthogroups_selection,
...
```{r}
orthogroups_selection<-read.csv("orthogroups_absrel_results.csv")

```

```{r}
table(orthogroups_selection$sex,
      orthogroups_selection$selection)
```

```{r}
# for now, remove those with nonsensical omegas (inspect/revisit later?)
orthogroups_selection<-orthogroups_selection[which(orthogroups_selection$omega<100),]
...
```{r, fig.show='hide'}
mod<-lm(orthogroups_selection$omega~orthogroups_selection$sex*orthogroups_selection$selection)
par(mfrow=c(2,2))
plot(mod)

anova(mod)

mod_aov<-aov(orthogroups_selection$omega~as.factor(orthogroups_selection$sex)*as.factor(orthogroups_selection$selection))
...
vioplot(orthogroups_selection$omega[orthogroups_selection$selection==1] ~ orthogroups_selection$sex[orthogroups_selection$selection==1],
        xlab="",
         ylab="",
        main="selection",
        col=sex_bias_colors,
...
axis(1,pos=0,at=0:3,labels=c("",
                             ...
                             paste0("UB\nn=", ns[3,2])),tick=FALSE)
# add letters
text(x=1:3,y=rep(15,3),
     c(labs[[3]]$Letters[["fem:1"]],
       labs[[3]]$Letters[["mal:1"]],
       labs[[3]]$Letters[["unbiased:1"]]), xpd=TRUE)

vioplot(orthogroups_selection$omega[orthogroups_selection$selection==0] ~ orthogroups_selection$sex[orthogroups_selection$selection==0],
        xlab="",
        ylab="",
        main="no selection", 
        col=sex_bias_colors,
...
axis(1,pos=0,at=0:3,labels=c("",
                             ...
                             paste0("UB\nn=", ns[3,1])),tick=FALSE)
# add letters
text(x=1:3,y=rep(3.25,3),
     c(labs[[3]]$Letters[["fem:0"]],
       labs[[3]]$Letters[["mal:0"]],
       labs[[3]]$Letters[["unbiased:0"]]), xpd=TRUE)

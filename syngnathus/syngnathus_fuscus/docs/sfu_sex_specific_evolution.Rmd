...
Now let's see if any of these genes are in the single copy groups
```{r}
single_copy_groups$S_fuscus_gene<-gsub("(g\\d+)\\.p\\d+","\\1",single_copy_groups$longest_orfs)

sex_specific_sc_orthologs<-merge(single_copy_groups,
                                 specific_genes,
                                 by.x="S_fuscus_gene",
                                 by.y = "Trinity_gene")
```

There are `r nrow(sex_specific_sc_orthologs)` sex-specific genes in the single-copy ortholog set. I wonder if it would be useful to investigate the types of genes (e.g., expression categories) of all the orthologs. 

```{r}
files<-list.files(path="06_diff_expr_analysis/01_high_exp_trinity_gene_names",
           pattern="biased",full.names = TRUE) 
biased_genes<-do.call(rbind,lapply(files,function(file){
  dat<-read.delim(file, header=FALSE)
  colnames(dat)<-"Trinity_gene"
  dat$sex <- gsub("^.*FU_(\\w{3})(\\w+).*$","\\1",file)
...
```{r}
specific_genes$DEG_cat<-"sex_specific"
biased_genes$DEG_cat<-"sex_biased"
degs<-rbind(specific_genes, biased_genes)
...
Now I will try aligning the single copy orthologues using PRANK.

...
pcaData <- plotPCA(vsd, intgroup = c("Organ", "Sex"), returnData = TRUE, ntop = nrow(dds_FL),pcsToUse=1:3)
pcaData$mate_success <- samples$mate_success
pcaData$repo_fit <- samples$rep_fittness

percentVar <- round(100 * attr(pcaData, "percentVar")) # adjust this too
...
ggplot(pca_plotting_data, aes(x=PC1, y=PC2, color = Sex, shape = Organ)) +
...
ggplot(pca_plotting_data, aes(x=PC1, y=PC3, color = Sex, shape = Organ)) +
  scale_color_manual(values = my_colors2) +
  scale_shape_manual(values = my_shapes2) +
  geom_point(aes(size = mate_success)) +
  xlab(paste0("PC1: ",percentVar[1],"% variance")) +
...
logFC_long_noNB <- logFC_long[logFC_long$bias != "NB",]
...
logFC_long_noNB$bias_cat <- ifelse(abs(logFC_long_noNB$logFC) >= 2 & abs(logFC_long_noNB$logFC) < 3, biased_bins[1],
                                   ifelse(abs(logFC_long_noNB$logFC) >= 3 & abs(logFC_long_noNB$logFC) < 5, biased_bins[2],
...
bias_cat_gill <- cbind(bias_cat_table[,, "Gill"], "sex-specific" = c(nrow(FL_sex_specific_genes$Gill_female_specific), nrow(FL_sex_specific_genes$Gill_male_specific)))
bias_cat_gonad <- cbind(bias_cat_table[,, "Gonad"], "sex-specific" = c(nrow(FL_sex_specific_genes$Gonad_female_specific), nrow(FL_sex_specific_genes$Gonad_male_specific)))
...
for(file_name in FL_blast_files){
  ...
  #Add column names to the dataset
  colnames(file_data) <- c("trin_geneid", "trin_gene_start", "trin_gene_end", "reiro_prot_info", "reiro_prot_name", "reiro_prot_start", "reiro_prot_end", "evalue", "bit_score", "length", "pident", "gaps")
...
blast_output_filtered <- lapply(FL_blast_list, function(data_frame){
  ...
  #Create an empty dataframe to store intermediate results in
  output <- data.frame(matrix(data = NA, ncol = ncol(data_frame), nrow = 0))
  colnames(output) <- c("trin_geneid", "trin_gene_start", "trin_gene_end", "reiro_prot_info", "reiro_prot_name", "reiro_prot_start", "reiro_prot_end", "evalue", "bit_score", "length", "pident", "gaps")
...
genes <- reiro_gff[reiro_gff$source == "BestRefSeq" & reiro_gff$feature == "CDS",]

#Pull out the gene name that corresponds to each protein ID
genes$gene_name <- gsub("^(.*;)(gene=)(\\w+);(.*$)", "\\3", genes$gene_info)
genes$prot_id <- gsub("^(.*;)(protein_id=)(.*)$", "\\3", genes$gene_info)

lapply(blast_output_filtered, function(dataframe){

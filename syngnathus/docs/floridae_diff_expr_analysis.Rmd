...
#This is a cohesiv list of all the libraries used in this document
library(DESeq2)
library(pheatmap)
...
I have then pulled out all of the male-biased, female-biased, and non-biased genes based on the criteria outlined above. I determined non-biased genes as a p-vaule > 0.05 AND a 2 > logFC > -2 as there are genes that could have a large FC but are not considered sig. due to outliers or other filtering employed by DESeq.
...
liver_non_biased <- liver_con_res_noNA[which(liver_con_res_noNA$log2FoldChange < 2 & liver_con_res_noNA$log2FoldChange > -2 & liver_con_res_noNA$padj > 0.05),]
...
#Creating an object that contains all of the non-biased genes in the gills, p>0.05 and 2>logFC>-2
gill_non_biased <- gill_con_res_noNA[which(gill_con_res_noNA$log2FoldChange < 2 & gill_con_res_noNA$log2FoldChange > -2 & gill_con_res_noNA$padj > 0.05),]
```
...
#Creating an object that contains all of the non-biased genes in the gonads, p>0.05 and 2>logFC>-2
gonad_non_biased <- gonad_con_res_noNA[which(gonad_con_res_noNA$log2FoldChange < 2 & gonad_con_res_noNA$log2FoldChange > -2 & gonad_con_res_noNA$padj > 0.05),]
```

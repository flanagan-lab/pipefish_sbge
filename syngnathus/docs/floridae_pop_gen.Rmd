...
# Tajima's D
After running ANGSD we can start to look through some of the results.

```{r SFS, fig.cap="Site frequency spectrum calculated by ANGSD."}
##Read in the data
...
#The variability as a percentile
pvar <- (1 - sfs[1] - sfs[length(sfs)])*100

#Plot it
barplot(sfs, names=1:length(sfs),
        legend=paste("Variability:= ",round(pvar,3),"%"),
        main='SFS', ylab = "Proportions",
        col = 'indianred4')

```

Question: Why does it go all the way to 57 when after 29 the proportions all come back as 0? What is on the x-axis since a transcriptome was used as a reference?
  - Here is what I have found:
    - Each bar in the barplot corresponds to a specific allele frequency category, and the height of the bar represents the count of sites or alleles falling into that frequency category.
    - How does this translate to the type of data that I am using?

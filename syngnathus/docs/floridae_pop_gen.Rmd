...
The following script was then used to filter the BAM files, estimate site frequency spectrum (SFS), calculate the thetas and the calculate Tajima's $D$ when `$ALLSFS=true and $THETAS=true`. It was then also used to filter BAM files and estimate SFS **separately** for males and females to all for a calculate of intersexual $F_{st}$ when `$INDSFS=true and $MFFST=true`.
